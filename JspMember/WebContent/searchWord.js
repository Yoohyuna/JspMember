/**
 * xhr객체를 생성->요청->jsp로 요청->응답->콜백함수 실시간으로 출력
 */
...
//3.파라미터값을 처리해주는 함수->메모리해제를 해주는 방법
function getParameterValues(){
	var mem_id=$("mem_id").value //document.regForm.mem_id.value(X)
	//서버캐시에 요청메모리에 저장하지 않는 방법->항상 동일한 url(X)
